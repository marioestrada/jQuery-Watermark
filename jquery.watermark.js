...
(function ($) {
    ...
    $.fn.watermark = function (text, options) {
        ...
        elems.each(function () {
            ...
            /*----------------------- show the outside parents of hidden elements ---------------
            ------------------------- &push the outside-most into an array -----------------------*/
            if (!$elem.is(":visible")) {
                var hidden = $elem.parents(":hidden");
                for (var i = 0; i < hidden.length; i++) {
                    $(hidden[i]).show();
                }
                hiddenArray.push($(hidden[(hidden.length) - 1]));
            }
            ...
            /*-------------- hide the elements that were pushed in the array before --------------*/
    ...
    $("input[placeholder]").watermark(undefined, {
        minOpacity: 0.0,
        animDuration: 0,
        fallback: true
    });
